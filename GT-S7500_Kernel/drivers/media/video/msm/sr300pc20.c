...
/* to upper layer. In case of this exposure time and ISO */
			...
			/* read exposure time */
			sr300pc20_sensor_read(0x20, &data);
			msb = (unsigned short)data;
			sr300pc20_sensor_read(0x21, &data);
			msb = (msb << 8) | (unsigned short)data;
			sr300pc20_sensor_read(0x22, &data);
			lsb = (unsigned short)data;
			sr300pc20_sensor_read(0x23, &data);
...
static int sr300pc20_regs_table_write(char *name)
{
	char *start = NULL, *end = NULL, *reg = NULL, *temp = NULL;/*,*data;*/
	...
	*(data_buf + 6) = '\0';
	...
	start = strnstr(sr300pc20_regs_table,
			name, sr300pc20_regs_table_size);
	if (start == NULL) {
