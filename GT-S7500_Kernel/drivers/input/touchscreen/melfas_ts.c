...
#define FW_VERSION					0x05	//model dependant
#else
...
static int check_delta_data(struct melfas_ts_data *ts)
{
	...
	for(i = 0; i < sensing_ch; i++ )
	{
		for (j = 0; j < exciting_ch; j++)
		{
			...
			ret = melfas_i2c_read(ts->client, UNIVERSAL_CMD_RESULT_SIZE, 1, read_buf);
...
//			mdelay(10);
			...
			ret = melfas_i2c_read(ts->client, UNIVERSAL_CMD_RESULT, read_data_len, read_data_buf);
...
{
	static const int NEW_FIRMWARE_VERSION = FW_VERSION; // 17 ?
...
//	}
printk("hunny : g_sensing_ch =%d, g_exciting_ch=%d\n",g_sensing_ch , g_exciting_ch );
	return ret;
}
...
struct device_attribute *attr, char *buf)
{
	...
	printk("[TSP] menukey : %d, backKey : %d\n", menuKey, backKey);
