...
static inline void hci_remote_ext_features_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (!ev->status && ev->page == 0x01) {
		...
		conn->ssp_mode = (ev->features[0] & 0x01);
                ...
                /*In case if remote device ssp supported/2.0 device reduce the security level to MEDIUM if it is HIGH : QC patch*/ 
                if (!conn->ssp_mode && (conn->pending_sec_level == BT_SECURITY_HIGH)) 
